...
async fn spawn_claude_process(app: AppHandle, mut cmd: Command, prompt: String, model: String, project_path: String) -> Result<(), String> {
    ...
    let stdout_task = tokio::spawn(async move {
        ...
        while let Ok(Some(line)) = lines.next_line().await {
            ...
            // Simple health event detection from message content
            if let Ok(msg) = serde_json::from_str::<serde_json::Value>(&line) {
                if let Some(content) = msg.get("message").and_then(|m| m.get("content")).and_then(|c| c.as_str()) {
                    let lc = content.to_lowercase();
                    let mut kind: Option<&str> = None;
                    if lc.contains("test") && (lc.contains("fail") || lc.contains("failed") || lc.contains("assert")) {
                        kind = Some("test_fail");
                    } else if lc.contains("test") && (lc.contains("pass") || lc.contains("passed") || lc.contains("ok (")) {
                        kind = Some("test_pass");
                    } else if lc.contains("git commit") || (lc.contains("committed") && lc.contains("git")) {
                        kind = Some("commit_pass");
                    }
                    if let Some(_k) = kind {
                        // Insert wellness event (best-effort, ignore errors)
                        // VibeGuard removed
                    }
                }
            }
